...
function processAndSavePersistence(signal, fs, outputDir, identifier)
    ...
    numSegments = floor((length(signal) - segmentLength) / overlap) + 1;
        ...
        persDir = fullfile(outputDir, 'persistance');
    if ~exist(persDir, 'dir')
    mkdir(persDir);
    end
    ...
    for i = 1:numSegments
        ...
        figure('Visible', 'off');
        pspectrum(segment, fs, "persistence");
        xlabel('');
        ylabel('');
        ...
        % Save the figure
        saveas(gcf, fullfile(persDir, sprintf('%s_persistence_%d.png', identifier, i)));
        close(gcf);
    end
end
